...
@injectable()
export class HarshBrakeController implements IBaseController {
    ...
    public register(app: express.Application): void {
        app.route('/harshBrake')
            .get(async (_request: Request, response: Response) => {
                ...
                await this._vehicleInformationService.getHarshBrake(from, to).then((harshBrake: IVehicleInformation[]) => {
                    response.json(harshBrake);
